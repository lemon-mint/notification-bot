	...
	if query := uq.withChannels; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ChannelsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "Channels": %w`, err)
		}
		query.Where(channel.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "Channels" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Channels = append(nodes[i].Edges.Channels, n)
			}
		}
	}
